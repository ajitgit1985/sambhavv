...
const Dashboard = () => {
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (ministryid && sectorCode) {
        ...
        apiUrl += `/mode?port_mode=${globalVar}`;
        console.log("port Code", globalVar);
      }
  ...
  const handleAPIMode = (selectedOption) => {
    ...
    setGlobalVar(selectedOption.value);
    //console.log("setGlobalVar", globalVar);
  };
  ...
  const renderView = () => {
    ...
    //   );
    // }
    return filteredData.map((item) => viewFunction(item));
  };
